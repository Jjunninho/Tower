...
function spawnBullet(charged) {
    ...
    // Valores padrão (atirar para frente se não houver boss)
    let vx = state.facing * spd;
    let vy = -1.5; 

    // TENTA MIRAR NO BOSS
    if (typeof BossSchool !== 'undefined' && BossSchool.currentBoss && BossSchool.currentBoss.active) {
        const boss = BossSchool.currentBoss;
        // Pega a posição central do boss (adaptado para os diferentes formatos de boss que você tem)
        const bx = boss.x ?? (boss.boss && boss.boss.x);
        const by = boss.y ?? (boss.boss && boss.boss.y);

        if (bx !== undefined) {
            // Calcula o ângulo entre o player e o boss
            const angle = Math.atan2(by - cy, bx - cx);
            
            // Define as velocidades X e Y baseadas no ângulo para "mirar"
    ...
    // Sons de disparo (mantidos do seu original)
    if (charged) {
    ...
    return {
        ...
        update() {
            ...
            for (let i = state.bullets.length - 1; i >= 0; i--) {
                ...
                // CORREÇÃO: compara posição do tiro em relação à câmera
                // b.y é coordenada de mundo; game.camera.y também é coordenada de mundo
                // relativeY = posição na tela (0 = topo, canvas.height = base)
                const relativeY = b.y - game.camera.y;

                if (hit || b.life <= 0 ||
                    b.x < -100 || b.x > canvas.width + 100 ||
                    relativeY < -200 ||
                    relativeY > canvas.height + 200) {
        ...
        draw(ctx) {
            if (game.gameOver) return;
            // NOTA: esta função deve ser chamada de DENTRO do bloco
            // que já aplicou ctx.translate(0, -game.camera.y) no 6_main.js.
            // Não aplica translate próprio para evitar dupla transformação.
            ...
            // Elementos de HUD precisam de coordenadas de tela.
            // Como o caller já fez translate, precisamos desfazê-lo
            // só para estes elementos e restaurar depois.
            ctx.save();
            ctx.translate(0, game.camera.y); // reverte o translate do caller
            ...
            ctx.restore(); // encerra o save do HUD acima

            // ── Indicador de combo de stomp ───────────────────
            // Desenhado dentro do translate(0, -camera.y) do caller,
            // então usa coords de mundo diretamente (sem subtrair camera.y)
            if (state.stomps > 0 && state.stompComboTimer > 0) {
                const screenX = player.x + player.width  / 2;
                const screenY = player.y - 30;   // world Y; translate do caller aplica câmera
