// --- UPDATE LOOP ---
	...
	// --- APLICAÇÃO DA GRAVIDADE (CORRIGIDO) ---
				...
				if (!player.onLadder) {
					...
					// ✅ NOVA LÓGICA: Na escada, mas permite pulo
					
					// Se apertar ESPAÇO, SOLTA da escada e pula
					if (game.keys[' '] && player.jumpsLeft > 0) {
						player.onLadder = false;  // ✅ Solta da escada
						player.vy = -player.jumpPower;  // ✅ Pula normalmente
						player.jumpsLeft--;
						audioSys.sfx.jump();
					} else {
						// Movimento vertical na escada (subir/descer)
						player.vy = 0;
				...
				player.onLadder = false;  // ✅ Reseta ANTES de verificar colisões
		...
		ladders.forEach(ladder => {
			// Verifica se o jogador está DENTRO da escada (não só tocando)
			const playerCenterX = player.x + player.width / 2;
			const playerBottom = player.y + player.height;
			
			// Só "gruda" na escada se:
			// 1. O centro do jogador está dentro da largura da escada
			// 2. O jogador está entre o topo e a base da escada (com margens)
			if (playerCenterX > ladder.x && 
				playerCenterX < ladder.x + ladder.width &&
				playerBottom > ladder.y + 10 &&  // ✅ Margem de 10px do topo
				player.y < ladder.y + ladder.height - 10) {  // ✅ Margem de 10px do fundo
				
				// ✅ SÓ ativa escada se jogador QUER subir/descer
            ...
            const currentHeight = Math.max(0, Math.floor((canvas.height - player.y + game.worldOffset) / 10));
