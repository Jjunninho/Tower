...
function loseLife() {
    // ADICIONE ISSO NO TOPO DA FUNÇÃO:
    if (player.hasShield) {
        audioSys.playTone(150, 'sine', 0.1); // Som de "bloqueio"
    ...
    if (game.lastCheckpoint) {
        ...
        player.y = game.lastCheckpoint.y;
        player.vx = 0;
        player.vy = 0;
...
function updateBootAI() {
    ...
    if (Math.random() < 0.005) {
		...
		boot.emotion = emotions[Math.floor(Math.random() * emotions.length)];
			}  // ← FECHA o if (Math.random() < 0.005)
		}      // ← FECHA a função updateBootAI();