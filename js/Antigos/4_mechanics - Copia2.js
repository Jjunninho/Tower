function bootSpeak(message) {
    ...
    setTimeout(() => { speechEl.style.display = 'none'; }, 2500);
}
...
function updateScore(points, reason) {
    ...
    if (game.combo > 1 && reason === 'height') showCombo();
}
...
function showCombo() {
    ...
    game.comboTimer = setTimeout(() => { comboEl.style.display = 'none'; }, 2000);
}
...
function endGame() {
    ...
    // Exibe seed no game over para o jogador poder compartilhar
    const seedEl = document.getElementById('finalSeed');
    if (seedEl) seedEl.textContent = game.seed || '(aleatória)';
...
function restartGame() {
    ...
    // ── Reseta o PRNG para o início da sequência ──────────────
    // Garante que ao reiniciar com a mesma seed, o mundo é idêntico
    if (typeof Rng !== 'undefined') Rng.reset();
...
function initLevel() {
    ...
    walls = []; lavaZones = []; enemies = []; escalators = [];
	    ...
	    // Reseta sistema de combate junto com o mundo
    if (typeof PlayerCombat !== 'undefined') PlayerCombat.init();
...
// ══════════════════════════════════════════════════════════
//  extendTower — geração procedural do mundo
//  Usa Rng.next() em vez de Math.random() para que a mesma
//  seed sempre produza o mesmo mundo.
//  Fallback: se Rng não estiver disponível, usa Math.random().
// ══════════════════════════════════════════════════════════
function extendTower() {
    // Helper: usa Rng se disponível, senão Math.random()
    const r = (typeof Rng !== 'undefined') ? () => Rng.next() : Math.random.bind(Math);
    ...
    while (currentY > targetY) {
        const spacing = 80 + r() * 100;
        currentY -= spacing;

        // 1. Boss tem prioridade
        ...
        if (currentY < game.nextCheckpointThreshold) {
            platforms.push({
                ...
                y: currentY,
                width: 100, height: 20,
                isCheckpoint: true, activated: false
            });
            game.nextCheckpointThreshold -= 3000;
            continue;
        }
        ...
        // ── Paredes laterais naturais ──────────────────────────
        // Aparecem com mais frequência conforme sobe (dificuldade)
        // Criam corredores e arenas semi-fechadas naturalmente
        const wallChance = 0.08 + (Math.abs(currentY) / 200000);
        if (r() < wallChance) {
            const side        = r() < 0.5 ? 0 : canvas.width - 40;
            const wallHeight  = 80 + r() * 120;
            walls.push({ x: side, y: currentY - wallHeight / 2, width: 40, height: wallHeight, placed: false });
        }

        if (type < 0.3) {
            // Plataforma normal
            const width = 100 + r() * 150;
            const x     = r() * (canvas.width - width);
            platforms.push({ x, y: currentY, width, height: 20 });

            if (r() < dangerLevel) {
                const lavaWidth = 60 + r() * 80;
            ...
            const width = 120;
            const x     = r() * (canvas.width - width);
            platforms.push({ x, y: currentY, width, height: 20, moving: true, startX: x, range: 200, speed: 1 + r() * 2 });
        } else if (type < 0.65) {
            // Escada
            const x = 50 + r() * (canvas.width - 150);
            ...
            const width = 150 + r() * 200;
            const x     = r() * (canvas.width - width);
            lavaZones.push({ x, y: currentY, width, height: 40 });
        }

        // Inimigos
        if (r() < 0.1 + (Math.abs(currentY) / 10000)) {
            const enemyX = r() * (canvas.width - 40);
            enemies.push({
                x: enemyX, y: currentY - 60, width: 40, height: 40,
                vx: (r() - 0.5) * 3,
                type: r() < 0.5 ? 'patrol' : 'chase',
...
function checkPlacementCollision(element) {
    ...
    const collisionBoxes = [
        ...
        ...escalators.map(e => ({
            ...
            width: Math.abs(e.x2 - e.x1), height: Math.abs(e.y2 - e.y1)
        })),
        ...enemies.map(e => ({ x: e.x, y: e.y, width: e.width, height: e.height }))
    ];

    elevators.forEach(e => {
        collisionBoxes.push({ x: e.x, y: e.startY - 200, width: e.width, height: 200 + e.height });
    });

    for (let existing of collisionBoxes) {
        if (checkOverlap(testBox, existing)) return false;
    }
    return true;
}
...
function updateBootAI() {
    ...
    let target = boot.history.length > boot.historyDelay
        ? boot.history[boot.history.length - boot.historyDelay]
        : player;
