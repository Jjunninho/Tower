// js/boss01.js - VERSÃO: BOOT ATIRADOR OTIMIZADO + BOSS TÁTICO BALANCEADO
...
const bossMatrix = {
    ...
    bootBullets: [],
    bootShootTimer: 0,
    bootShootRate: 4,  // Atira a cada 4 frames (~15 tiros/segundo) - BEM RÁPIDO!
    isBootShooting: true,
    bootHitCount: 0,
    totalDamage: 0,
    ...
    update: function() {
        ...
        for (let i = this.bootBullets.length - 1; i >= 0; i--) {
            ...
            const hitRadius = (b.headRadius + 20) * (b.headRadius + 20);
            
            if (distSq < hitRadius) {
                // ACERTOU!
                const damage = 2.5; // DANO FORTE!
                b.health -= damage;
                b.isHit = true;
                b.hitFlashTimer = 3;
        ...
        // === 5. ATAQUES DO BOSS (MAIS LENTO E MENOS PRECISO) ===
        b.shootTimer++;
        ...
        if (this.playerIsStill) {
            // PLAYER PARADO = Boss um pouco mais perigoso
            shootInterval = 120;  // Mais lento que antes
            inaccuracy = 60;      // Menos preciso
        } else {
            // PLAYER SE MOVENDO = Boss bem fraco
            shootInterval = 180;  // Muito lento
            inaccuracy = 300;     // Muito impreciso
        }
        
        if (b.shootTimer > shootInterval) {
            this.shootLaser(inaccuracy);
            
            // Projéteis apenas quando está quase morto
            if (b.health < 30 && this.playerIsStill) {
	...
	shootBootBullet: function() {
		...
		const spread = (Math.random() - 0.5) * 0.03; // Bem preciso
		const finalAngle = angle + spread;

		const speed = Math.min(15, Math.max(11, dist / 22)); // Mais rápido
