...
const PlayerCombat = (() => {
    ...
    function spawnBullet(charged) {
        // Origem na "mão" do player: frente se olha à direita, trás se olha à esquerda
        // Altura: 1/3 do corpo (linha dos ombros, não do centro)
        const cx  = state.facing === 1
            ? player.x + player.width + 2          // mão direita
            : player.x - 2;                        // mão esquerda
        const cy  = player.y + player.height * 0.3; // altura dos ombros
        // Herda a velocidade lateral do player para não "descolar" em movimento
        const inheritVx = player.vx * 0.4;
        const spd = charged ? CFG.chargeSpeed : CFG.bulletSpeed;

        state.bullets.push({
            x:       cx,
            y:       cy,
            vx:      state.facing * spd + inheritVx,
