// ===== js/3_biomes.js =====
// SISTEMA DE BIOMAS PROCEDURAIS INFINITOS — Geração por Chunk, Física, Arte e UI

// ════════════════════════════════════════════════════════════════
// 1. MOTOR PROCEDURAL INFINITO (Arquétipos + Chunks + Seed)
// ════════════════════════════════════════════════════════════════

const CHUNK_SIZE      = 3000; // metros por bioma
const TRANSITION_ZONE =  500; // últimos 500m do chunk fazem blend

const BIOME_ARCHETYPES = [
    { id: 'UNDERGROUND', name: 'Subterrâneo',       color: '#8B4513' },
    { id: 'GROUND',      name: 'Superfície',         color: '#4A90E2' },
    { id: 'FOREST',      name: 'Floresta Flutuante', color: '#00ff44' },
    { id: 'CLOUDS',      name: 'Reino das Nuvens',   color: '#B0E0E6' },
    { id: 'TWILIGHT',    name: 'Crepúsculo',         color: '#cc44ff' },
    { id: 'SPACE',       name: 'Espaço Sideral',     color: '#8866ff' },
    { id: 'COSMIC',      name: 'Vazio Cósmico',      color: '#00ffcc' },
];
...
// ════════════════════════════════════════════════════════════════
// 3. QUERY DE BIOMA — Procedural Infinita por Chunk + Seed
// ════════════════════════════════════════════════════════════════

function _pickArchetype(chunk, seed) {
    const idx = Math.floor(_hash(chunk, seed) * BIOME_ARCHETYPES.length);
    return BIOME_ARCHETYPES[idx] || BIOME_ARCHETYPES[0];
}

function _getGameSeed() {
    if (typeof game !== 'undefined' && game.seed) {
        const s = game.seed.toString();
        return s.length > 0 ? s.charCodeAt(0) : 1337;
    }
    return 1337;
}

function getCurrentBiome() {
    const altitude = (typeof game !== 'undefined' && game.camera && !isNaN(game.camera.y))
        ? Math.max(0, -game.camera.y)
        : 0;

    const seed    = _getGameSeed();
    const chunk   = Math.floor(altitude / CHUNK_SIZE);

    // Sorteia bioma atual e o próximo via hash determinístico
    const current = _pickArchetype(chunk,     seed);
    const next    = _pickArchetype(chunk + 1, seed);

    // Distância até a borda do chunk atual
    const distToNext = CHUNK_SIZE - (altitude % CHUNK_SIZE);

    // Blend suave apenas dentro da zona de transição
    const blend = (distToNext < TRANSITION_ZONE)
        ? _easeInOut(1 - _clamp(distToNext / TRANSITION_ZONE, 0, 1))
        : 0;

    const active = blend > 0.5 ? next : current;
